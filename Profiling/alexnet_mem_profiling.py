...
def alexnet_predict(resized, model):
    prediction_array = model.predict(np.expand_dims(resized, axis = 0))
            ...
            while predict_calls < max_predict:
                ...
                if hand_landmarks:
                    ...
                    if(out_of_bounds(coord)):
                        ...
                        letter, letter_w_percent = alexnet_predict(resized, model)
                        predict_calls += 1
...
# with open('alexnet_time_profiling.txt','w') as f:
